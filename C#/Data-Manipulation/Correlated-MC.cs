...
namespace Project3
{
    class Program
    {
        ...
        {
            double[,,] stock = new double[4, 20, 756];
            stock = Read_and_Calc();
            string[] stock_return = Average_Return(stock);
            
            Console.WriteLine("Simulated annualized returns are: ");
            Console.WriteLine("ATVI\tNFLX\tPFE\tTSLA");
            for (int i = 0; i < 4; i++)
            {
                Console.Write(stock_return[i] + "\t");
            }
        ...
        static double[,,] Read_and_Calc()
        {
            ...
            double[,,] rnd = RandomPath(4, 20, 755);

            double[,,] rnd_mc = new double[4, 20, 755];
            for (int t = 0; t < 20; t++)
            {
                for (int i = 0; i < 4; i++)
                {
            ...
            double[,,] stock_mc = new double[4, 20, 756];
        ...
        static string[] Option(double[,,] stock_path)
        {
            ...
            for (int i = 0; i < 4; i++)
            {
                double[] one_trial = new double[20];
                for (int j = 0; j < 20; j++)
                {
                    payoff[i] += Math.Max(0, stock_path[i, j, 755] - strike[i])/20;
                }
